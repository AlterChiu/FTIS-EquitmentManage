...
namespace DouImp.Controllers.Manager
{
    ...
    public class UserController : UserBaseControll<User, Role>
    {
        ...
        bool IsSsoAuth = true;
        const string SsoServer = "https://pj4.ftis.org.tw/Sso/";
        ...
        public ActionResult Login(User user, string returnUrl, bool redirectLogin = false, bool sso = true)
        {
                ...
                if (_ssotoken != null)
                {
                    ...
                    if ((bool)ssou.Success)
                    {
                        ...
                        if (u != null)
                        {
                            ...
                            //if (ssoudc != u.Dep || !Dou.Context.Config.VerifyPassword(u.Password, pw))
                            {
                                u.Dep = ssoudc;
                                u.EMail = ssouemail;
                        ...
                        else//系統尚無此使用者,寫入此使用者資料
                        {
                            ...
                            //user = new User() { Id = ssouid, Name = ssouname, Password = Dou.Context.Config.PasswordEncode(pw), Dep = ssoudc, Enabled = false };
            ...
            if (ViewBag.ErrorMessage == null && user.Id != null && !User.Identity.IsAuthenticated)//login remember me
            {
                var identity = new ClaimsIdentity(new[] {new Claim("Id", user.Id)
                                    }, DouImp.Startup.ApplicationCookieAppName);
            ...
            else
            {
                if (v is RedirectResult || v is RedirectToRouteResult)
                {
                    ...
                    //DouUnobtrusiveSession.Session.Add("CurrentFtisUser", FtisHelper.DB.FtisModelContext.GetEmployeeIncludeDepartment(user.Id));
                }
        ...
        public override ActionResult DouLogoff()
        {
            ...
            HttpContext.GetOwinContext().Authentication.SignOut(DouImp.Startup.ApplicationCookieAppName); //清除login remember me
